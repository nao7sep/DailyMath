namespace DailyMath.Core.Windows.Tests.Rendering;

using DailyMath.Core.Rendering;
using DailyMath.Core.Windows.Rendering;

public class WpfTextMeasurerTests
{
    ...
    public void MeasureText_EmptyStringHasNonZeroHeight()
    {
        ...
        // Even empty text should have height (font's line height)
        Assert.True(size.Height > 0);
    }
    ...
    [Fact]
    public void WpfDipResolution_Is96()
    {
        Assert.Equal(96.0, WpfTextMeasurer.WpfDipResolution);
    }

    [Fact]
    public void MeasureText_UsesInvariantCulture()
    {
        // Test that measurements are culture-independent
        var spec = new FontSpec("Arial", 12);
        var size1 = _measurer.MeasureText("1234.56", spec);

        // Measurement should be consistent regardless of current culture
        Assert.True(size1.Width > 0);
        Assert.True(size1.Height > 0);
    }
    ...
    public void MeasureText_AllFontWeightsMapped(FontWeight weight, int expectedValue)
    {
        // Verify that our FontWeight enum matches expected values
        Assert.Equal(expectedValue, (int)weight);

        // And that they all work in measurement
        var spec = new FontSpec("Arial", 12, weight);
        var size = _measurer.MeasureText("Test", spec);
        Assert.True(size.Width > 0);
    }
    ...
    [Fact]
    public void MeasureText_ExtentGivesTightBounds()
    {
        var spec = new FontSpec("Arial", 12);
        var size = _measurer.MeasureText("Hpg", spec);

        // Using Extent should give us tight bounds (no extra leading)
        // Height should be reasonable for 12pt font
        Assert.True(size.Height > 10);
        Assert.True(size.Height < 20); // Should be tighter than including line gap
    }

    [Fact]
    public void MeasureText_DipToPointConversionAccurate()
    {
        // Test DIP to point conversion: points = DIPs * 72 / 96 = DIPs * 0.75
        var spec12 = new FontSpec("Arial", 12); // 12 points = 16 DIPs
        var spec16 = new FontSpec("Arial", 16); // 16 points ≈ 21.33 DIPs

        var size12 = _measurer.MeasureText("X", spec12);
        var size16 = _measurer.MeasureText("X", spec16);

        // Should scale approximately by 16/12 = 1.33
        double ratio = size16.Height / size12.Height;
